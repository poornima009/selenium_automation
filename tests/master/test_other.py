...
LIST = master_entities(group="other", kind="list")
VIEW = master_entities(group="other", kind="view")


@pytest.mark.parametrize("entity", LIST, ids=lambda e: e.key)
def test_master_other_add_update_approve(
    settings: Settings,
