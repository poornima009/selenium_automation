...
from automation.flows.master.client_flow import ClientCreateBlockedError, MasterClientFlow
...
from automation.pages.store.master.client_page import ClientRecord
...
def test_master_client_add_search_update_approve_track(
    settings: Settings,
    store_session: tuple[Page, StoreWorkspacePage],
) -> None:
    stamp = datetime.now().strftime("%Y%m%d%H%M%S")
    record = ClientRecord(
        name=f"AUTO-CLIENT-{stamp}",
        pan_number=f"CLICL{stamp[-4:]}X",
        contact_person=f"Person {stamp[-6:]}",
        email=f"client{stamp[-6:]}@example.com",
        contact_number=f"9{stamp[-9:]}",
        bank=BankDetails(
            account_number=stamp[-12:],
            account_holder=f"Client {stamp[-6:]}",
        ),
    )
    ...
    flow = MasterClientFlow(page, settings)
    try:
        listing = flow.create(record, remark="AUTO Client create")
    except ClientCreateBlockedError as error:
    ...
    flow.update(record.name, updated_contact, remark="AUTO Client update")
    assert flow.approve(record.name, remark="AUTO Client approve")
