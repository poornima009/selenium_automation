...
from automation.flows.master.contractor_flow import (
    ContractorCreateBlockedError,
    MasterContractorFlow,
)
...
from automation.pages.store.master.contractor_page import ContractorRecord
from automation.pages.store.store_workspace_page import StoreWorkspacePage

CONTRACTOR_MODULE = "Contractor"
CRUD_OPERATIONS = ("add", "update", "approve")


def test_master_contractor_add_search_update_approve_track(
    settings: Settings,
    store_session: tuple[Page, StoreWorkspacePage],
) -> None:
    stamp = datetime.now().strftime("%Y%m%d%H%M%S")
    record = ContractorRecord(
        name=f"AUTO-CONTRACTOR-{stamp}",
        pan_number=f"AUTCC{stamp[-4:]}X",
        contact_person=f"Person {stamp[-6:]}",
        email=f"contractor{stamp[-6:]}@example.com",
        contact_number=f"9{stamp[-9:]}",
        bank=BankDetails(
            account_number=stamp[-12:],
            account_holder=f"Contractor {stamp[-6:]}",
        ),
    )
    ...
    for operation in CRUD_OPERATIONS:
        assert permissions.is_granted(CONTRACTOR_MODULE, operation)

    flow = MasterContractorFlow(page, settings)
    try:
        listing = flow.create(record, remark="AUTO Contractor create")
    except ContractorCreateBlockedError as error:
    ...
    flow.update(record.name, updated_contact, remark="AUTO Contractor update")
    assert flow.approve(record.name, remark="AUTO Contractor approve")
