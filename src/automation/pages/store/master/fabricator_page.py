...
class MasterFabricatorPage(BasePage):
    """Master > Account > Fabricator form fields."""

    FABRICATOR_MAPPING = "label.MuiFormControlLabel-root:has-text('Fabricator')"
    PAN_NOT_LINKED = "button:text-is('No')"

    def fill_create(self, record: FabricatorRecord) -> None:
        logger.info("Filling Fabricator create form for %s", record.name)
        self.fill_labeled("Fabricator Name", record.name)
        ...
        self._select_fabricator_mapping()
    ...
    def fill_update(self, contact_person: str) -> None:
        logger.info("Updating Fabricator contact person to %s", contact_person)
    ...
    def fill_remark(self, remark: str) -> None:
        self.fill_optional_remark(remark, "Fabricator")
    ...
    def _select_fabricator_mapping(self) -> None:
        mapping = self.locator(self.FABRICATOR_MAPPING)
        checkbox = mapping.locator("input[type='checkbox']")
        if checkbox.is_checked():
            return
        logger.info("Selecting Fabricator account mapping")
        mapping.click()
