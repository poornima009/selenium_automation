...
class MasterClientPage(BasePage):
    """Master > Account > Client form fields."""

    CLIENT_MAPPING = "label.MuiFormControlLabel-root:has-text('Client')"
    PAN_NOT_LINKED = "button:text-is('No')"

    def fill_create(self, record: ClientRecord) -> None:
        logger.info("Filling Client create form for %s", record.name)
        self.fill_labeled("Client Name", record.name)
    ...
    def fill_remark(self, remark: str) -> None:
        self.fill_optional_remark(remark, "Client")
    ...
    def _select_client_mapping(self) -> None:
        mapping = self.locator(self.CLIENT_MAPPING)
        checkbox = mapping.locator("input[type='checkbox']")
        if checkbox.is_checked():
            return
        logger.info("Selecting Client account mapping")
        mapping.click()
