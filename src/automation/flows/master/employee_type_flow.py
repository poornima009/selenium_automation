...
from automation.pages.store.master.employee_type_page import (
    EmployeeTypeRecord,
    MasterEmployeeTypePage,
)
...
class EmployeeTypeCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Employee Type creation."""


class MasterEmployeeTypeFlow:
    """Create, update, approve, and track an Employee Type."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterEmployeeTypePage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("employee_type").path)
    ...
    def create(self, record: EmployeeTypeRecord, remark: str) -> MasterListPage:
        logger.info("Creating Employee Type %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise EmployeeTypeCreateBlockedError(
                "Application bug: Employee Type confirmation Save leaves the "
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Employee Type %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Employee Type %s", name)
        listing = self._list
