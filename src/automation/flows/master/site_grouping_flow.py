...
from automation.pages.store.master.site_grouping_page import (
    MasterSiteGroupingPage,
    SiteGroupingRecord,
)
...
class SiteGroupingCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Site Grouping creation."""


class MasterSiteGroupingFlow:
    """Create, update, approve, and track a Site Grouping."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterSiteGroupingPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("site_grouping").path)
    ...
    def create(self, record: SiteGroupingRecord, remark: str) -> MasterListPage:
        logger.info("Creating Site Grouping %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise SiteGroupingCreateBlockedError(
                "Application bug: Site Grouping confirmation Save leaves the "
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Site Grouping %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Site Grouping %s", name)
        listing = self._list
