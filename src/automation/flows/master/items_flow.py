...
from automation.pages.store.master.items_page import ItemRecord, MasterItemsPage
...
class ItemCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Item creation."""


class MasterItemsFlow:
    """Create, update, approve, and track an Item."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterItemsPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("items").path)
    ...
    def create(self, record: ItemRecord, remark: str) -> MasterListPage:
        logger.info("Creating Item %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise ItemCreateBlockedError(
                "Application bug: Item confirmation Save leaves the dialog "
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Item %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Item %s", name)
        listing = self._list
