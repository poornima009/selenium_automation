...
from automation.pages.store.master.contractor_page import (
    ContractorRecord,
    MasterContractorPage,
)
...
class ContractorCreateBlockedError(RuntimeError):
    """The known Contractor confirmation defect prevented record creation."""


class MasterContractorFlow:
    """Create, update, approve, and track a Contractor in one Store session."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterContractorPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("contractor").path)
    ...
    def create(self, record: ContractorRecord, remark: str) -> MasterListPage:
        logger.info("Creating Contractor %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise ContractorCreateBlockedError(
                "Known application bug: Contractor confirmation Save leaves the "
    ...
    def update(self, name: str, contact_person: str, remark: str) -> MasterListPage:
        logger.info("Updating Contractor %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Contractor %s", name)
        listing = self._list
