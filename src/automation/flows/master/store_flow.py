...
from automation.pages.store.master.store_page import MasterStorePage, StoreRecord
from automation.pages.store.store_nav_page import StoreNavPage

logger = get_logger(__name__)


class MasterStoreFlow:
    """Add one Store, then search / open / action / remark without leaving the session."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterStorePage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("store").path)
    ...
    def create(self, record: StoreRecord, remark: str) -> MasterListPage:
        logger.info("Creating Store %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        listing.search(record.name)
        return listing

    def update(self, name: str, alias: str, remark: str) -> MasterListPage:
        logger.info("Updating Store %s", name)
        listing = self.find(name)
        listing.open_row(name)
        self._form.fill_update(alias)
        self._form.fill_remark(remark)
        listing.save()
        listing.confirm(remark=remark)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Store %s", name)
        listing = self._list
