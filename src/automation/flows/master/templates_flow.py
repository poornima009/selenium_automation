...
from automation.pages.store.master.templates_page import (
    MasterTemplatesPage,
    TemplateRecord,
)
...
class TemplateCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Template creation."""


class MasterTemplatesFlow:
    """Create, update, approve, and track a Template."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterTemplatesPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("templates").path)
    ...
    def create(self, record: TemplateRecord, remark: str) -> MasterListPage:
        logger.info("Creating Template %s", record.name)
        listing, mapped_categories = self._open_with_mapped_categories()
        listing.open_add()
        self._form.fill_create(record, mapped_categories)
        self._form.fill_remark(remark)
        listing.save()
        listing.confirm(remark=remark)
        if listing.visible_dialog() is not None:
            raise TemplateCreateBlockedError(
                "Template confirmation remains open after the create attempt"
            )
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Template %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Template %s", name)
        listing = self._list
    ...
    def _open_with_mapped_categories(
        self,
    ) -> tuple[MasterListPage, set[str]]:
        mapped_categories: set[str] = set()

        def capture(response: Response) -> None:
            if "/template/get/allTemplates" not in response.url:
                return
            payload = response.json()
            rows = payload.get("data", {}).get("data", [])
            for row in rows:
                mappings = row.get("categoryCompanyEntityTemplateMappings", [])
                mapped_categories.update(
                    mapping["categoryName"]
                    for mapping in mappings
                    if mapping.get("categoryName")
                )
        ...
        return listing, mapped_categories
