...
from automation.pages.store.master.unit_page import MasterUnitPage, UnitRecord
from automation.pages.store.store_nav_page import StoreNavPage

logger = get_logger(__name__)


class UnitCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Unit creation."""


class MasterUnitFlow:
    """Create, update, approve, and track a Unit in one session."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterUnitPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("unit").path)
    ...
    def create(self, record: UnitRecord, remark: str) -> MasterListPage:
        logger.info("Creating Unit %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise UnitCreateBlockedError(
                "Application bug: Unit confirmation Save leaves the dialog "
                "open and sends no create request"
            )
    ...
    def update(
        ...
        logger.info("Updating Unit %s", name)
        listing = self.find(name)
        listing.open_row(name)
        self._form.fill_update(updated_name, abbreviation)
        self._form.fill_remark(remark)
        listing.save()
        listing.confirm(remark=remark)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Unit %s", name)
        listing = self._list
