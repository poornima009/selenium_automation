...
class AccountDepartmentsCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Account Departments creation."""


class MasterAccountDepartmentsFlow:
    """Create, update, approve, and track Account Departments."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterAccountDepartmentsPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("departments").path)
    ...
    def create(
        ...
        logger.info("Creating Account Department %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise AccountDepartmentsCreateBlockedError(
                "Application bug: Account Departments confirmation Save leaves "
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Account Department %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Account Department %s", name)
        listing = self._list
