...
from automation.pages.store.master.terms_page import MasterTermsPage, TermsRecord
from automation.pages.store.store_nav_page import StoreNavPage

logger = get_logger(__name__)


class TermsCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented T&C creation."""


class MasterTermsFlow:
    """Create, update, approve, and track T&C."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterTermsPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("tandc").path)
    ...
    def create(self, record: TermsRecord, remark: str) -> MasterListPage:
        logger.info("Creating T&C %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise TermsCreateBlockedError(
                "Application bug: T&C confirmation Save leaves the dialog open "
    ...
    def update(
        ...
        logger.info("Updating T&C %s", name)
        listing = self.find(name)
        listing.open_row(name)
        self._form.fill_update(updated_name, content)
        self._form.fill_remark(remark)
        listing.save()
        listing.confirm(remark=remark)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving T&C %s", name)
        listing = self._list
