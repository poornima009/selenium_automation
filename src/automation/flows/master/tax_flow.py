...
from automation.pages.store.master.tax_page import MasterTaxPage, TaxRecord
from automation.pages.store.store_nav_page import StoreNavPage

logger = get_logger(__name__)


class TaxCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Tax creation."""


class MasterTaxFlow:
    """Create, update, approve, and track a Tax."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterTaxPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("tax").path)
    ...
    def create(self, record: TaxRecord, remark: str) -> MasterListPage:
        logger.info("Creating Tax %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise TaxCreateBlockedError(
                "Application bug: Tax confirmation Save leaves the dialog open "
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Tax %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Tax %s", name)
        listing = self._list
