...
from automation.pages.store.master.designation_page import (
    DesignationRecord,
    MasterDesignationPage,
)
...
class DesignationCreateBlockedError(RuntimeError):
    """The confirmation dialog prevented Designation creation."""


class MasterDesignationFlow:
    """Create, update, approve, and track a Designation in one session."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterDesignationPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("designation").path)
    ...
    def create(self, record: DesignationRecord, remark: str) -> MasterListPage:
        logger.info("Creating Designation %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise DesignationCreateBlockedError(
                "Application bug: Designation confirmation Save leaves the "
    ...
    def update(self, name: str, updated_name: str, remark: str) -> MasterListPage:
        logger.info("Updating Designation %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Designation %s", name)
        listing = self._list
