...
from automation.pages.store.master.transporter_page import (
    MasterTransporterPage,
    TransporterRecord,
)
...
class TransporterCreateBlockedError(RuntimeError):
    """The known account confirmation defect prevented Transporter creation."""


class MasterTransporterFlow:
    """Create, update, approve, and track a Transporter in one Store session."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterTransporterPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("transporter").path)
    ...
    def create(self, record: TransporterRecord, remark: str) -> MasterListPage:
        logger.info("Creating Transporter %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise TransporterCreateBlockedError(
                "Known application bug: Transporter confirmation Save leaves the "
    ...
    def update(self, name: str, contact_person: str, remark: str) -> MasterListPage:
        logger.info("Updating Transporter %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Transporter %s", name)
        listing = self._list
