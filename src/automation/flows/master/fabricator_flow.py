...
from automation.pages.store.master.fabricator_page import FabricatorRecord, MasterFabricatorPage
...
class FabricatorCreateBlockedError(RuntimeError):
    """The known account confirmation defect prevented Fabricator creation."""


class MasterFabricatorFlow:
    """Create, update, approve, and track a Fabricator in one Store session."""
    ...
    def __init__(self, page: Page, settings: Settings | None = None) -> None:
        ...
        self._form = MasterFabricatorPage(page, self._settings)

    def open(self) -> MasterListPage:
        self._list.dismiss_overlay()
        self._nav.open_path(*master_entity("fabricator").path)
    ...
    def create(self, record: FabricatorRecord, remark: str) -> MasterListPage:
        logger.info("Creating Fabricator %s", record.name)
        listing = self.open()
        listing.open_add()
        ...
        if listing.visible_dialog() is not None:
            raise FabricatorCreateBlockedError(
                "Known application bug: Fabricator confirmation Save leaves the "
    ...
    def update(self, name: str, contact_person: str, remark: str) -> MasterListPage:
        logger.info("Updating Fabricator %s", name)
    ...
    def approve(self, name: str, remark: str) -> bool:
        logger.info("Approving Fabricator %s", name)
        listing = self._list
